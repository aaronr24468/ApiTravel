...
import { findTripByTransferId, finishTripUpdate, getPaymentStatus, getPriceTripSeats, paidSucceededUser, paymentExist, refundExist, updateDataTrip, updateSeatsStatus, updateStatusRefound, updateWebhookTransferDriver } from "../models/trip.models.mjs";
import { tripCompleted, updateOnboardingState } from "../models/user.models.mjs";
import { refundExpiredReservation } from "../services/refundExpired.mjs";
config();
...
export const stripeWebHook = async (req, res) => {
    ...
    switch (event.type) {
        case "payment_intent.succeeded": {
            ...
            const statusP = await getPaymentStatus(data);


            if (statusP[0].payment_status === "expired") {

                data.payment_status = "expired";

                await paidSucceededUser(data)

                refundExpiredReservation(data) //revizar que se este realizando el rembolso exitosamente,
                break;
            }
        ...
        case "transfer.created": {
            ...
            if (payout[0].payout === "paid") return


            const tranWEb = await updateWebhookTransferDriver(idPayout);
            //actualizamos estus de pago del conductor a pagado

            const tripCR = await tripCompleted(idTrip);
            break;
        }
