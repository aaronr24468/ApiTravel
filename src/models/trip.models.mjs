...
export const updateSeatsStatus = async(data) =>{
    const query = 'UPDATE trips SET available_seats=? WHERE id=?';
    const [res] = await connectionDB.query(query, [data.seats, data.id]);
    return(res)
}
...
export const findTripByTransferId = async(tranferId) =>{
    const query = 'SELECT idPayout FROM trips WHERE idPaylut=?';
    const [result] = await connectionDB.query(query, [tranferId]);
    return(result)
}
