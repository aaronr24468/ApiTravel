...
export const saveCarInfo = async(data) =>{
    ...
    const responseDB = await connectionDB.query(query, [data.driver_id, data.brand, data.model, data.color, data.plates, data.insured, data.year, data.seats, data.img_Vehicle]);
    return(responseDB) 
}
